...
def calc_dist(n1, n2, width_m=100.0, height_m=60.0):
    dx = (n1.coord_x - n2.coord_x) * width_m
    dy = (n1.coord_y - n2.coord_y) * height_m
...
def seed_db():
    ...
    try:
        ...
        shopping = Shopping(
            nome="Shopping Demonstração Center",
        ...
        for u, v in arestas_conexoes:
            ...
            a1 = Aresta(origem_id=n1.id, destino_id=n2.id, distancia=dist, bidirecional=True, acessivel=acessivel, ativa=ativa)
            arestas.append(a1)
        ...
        for token, nid in qrcodes_data:
            q = QRCode(
                token=token,
                no_id=n_map[nid].id,
                descricao=f"QR Code para {token}",
                ativo=True
            )
            qrcodes.append(q)
        db.add_all(qrcodes)
